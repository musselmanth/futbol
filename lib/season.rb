class Season
  ...
  def self.generate_seasons(games)
    seasons_hash = {}
    games.each do |game|
      if !seasons_hash.keys.include?(game.season)
        seasons_hash[game.season] = Season.new(game.season)
      end
      seasons_hash[game.season].games_in_season << game
    end
    seasons_hash
  end 
end
